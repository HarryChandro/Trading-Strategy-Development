...
def full_data():
    try:
        ...
        end_date = datetime.now()  # no +7 days
...
API_KEY = "PKTMQKIAUQRRLCKIP5MP"
API_SECRET = "7U39rBlxGBeMGFpv5kcUAQq2g72agY2AAeSuuD0d"
BASE_URL = "https://paper-api.alpaca.markets"

# Correct: do not add api_version='v2'
...
print("Starting Balance:", account.cash)
...
# =========================
# Step 1: Determine the best strategy dynamically
# =========================
if ema_result['return_pct'] > optimal_results['total_return']:
    strategy = 'EMA'
    best_return_value = ema_result['return_pct']
else:
    strategy = 'RSI'
    best_return_value = optimal_results['total_return']

print(f"✅ Best strategy selected: {strategy} (Return: {best_return_value:.2f}%)")


# =========================
# Step 2: Function to get latest signal
# =========================
def get_latest_signal(data, strategy):
    if strategy == 'EMA':
        ma_sm = data['Close'].ewm(span=sm, adjust=False).mean()
        ma_lg = data['Close'].ewm(span=lg, adjust=False).mean()

        if ma_sm.iloc[-1] > ma_lg.iloc[-1] and ma_sm.iloc[-2] <= ma_lg.iloc[-2]:
            return "buy"
        elif ma_sm.iloc[-1] < ma_lg.iloc[-1] and ma_sm.iloc[-2] >= ma_lg.iloc[-2]:
            return "sell"
        else:
            return "hold"
        ...
        if rsi < best_oversold:
            return "buy"
        elif rsi > best_overbought:
            return "sell"
        else:
            return "hold"
...
# =========================
signal = get_latest_signal(data, strategy)
print(f"📌 Latest signal based on {strategy}: {signal.upper()}")


# =========================
# Step 4: Execute trade & send one Telegram message
# =========================
message = f"📊 <b>{ticker} Update</b>\nStrategy: {strategy}\nSignal: {signal.upper()}"

try:
    positions = api.list_positions()
    held_symbols = [pos.symbol for pos in positions]
    has_position = ticker in held_symbols

    latest_close = float(data['Close'].iloc[-1])
    trade_amount = 200
    qty = int(trade_amount / latest_close)
    ...
    if signal == "buy" and not has_position:
        ...
        message += f"\n\n🤝 HOLD — No trade action taken."

    else:
        message += f"\n\n⚠️ No action — already in position or invalid signal."
